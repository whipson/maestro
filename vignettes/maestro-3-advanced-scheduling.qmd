...
This vignette covers more advanced concepts and examples related to scheduling. At its core, maestro adheres to two key principles:
...
All three pipelines are hourly but they start on different 10-minute intervals. If we run the orchestrator at `1 hour` they'll all execute at the same time. If it is important that they execute at different times, we should set it to `10 minutes`.
