...
#### Manual Approach
From either the *Project* drop down (typically the upper right) or the *File* tab (typically the upper left)

**Project drop down**

- In the *New Project Wizard* window select *New Directory*
- Scroll down and select *Maestro Project*
- In the *Directory name* input box provide a project name (i.e. `maestro_project`)
- Browse to the location to where the project will be created
- In the *Orchestrator  Type* drop down select the format (i.e `R`, `Quatro` or `RMarkdown`)
...
## Create Pipelines
Creating pipelines is based on the following `maestro` decorators:
...
| log_level | The logging level for the pipeline to be used if logging is is identified in the orchestrator script (e.g., INFO, WARN, ERROR) | `ERROR` |
| open | whether or not to open the script upon creation | `TRUE` |
...
| n_show_next| show the next n number of scheduled pipes | `5` | `5` |
| cores | Number of cpu cores to run if running in parallel | `1` | `1` |
| logging | whether or not to write the logs to a file | `FALSE` | `TRUE` |
| log_file | path to the log file | `./maestro.log` |  `./maestro_pipeline.log` |
| log_file_max_bytes | numeric specifying the maximum number of bytes allowed in the log file before purging the log | `1e+06` | `10485760` |
