...
-   `#' @maestroInputs collect(multiply)` *(collect all iterations of an `each()` upstream)*

## maestroIterateOver

For use with `each()` in `@maestroInputs`. Specifies one or more `.input$field` expressions (space-separated) to scatter over. Each expression is evaluated against the upstream return value and the resulting vectors are zipped together `pmap`-style: each iteration receives `.input` with all specified fields replaced by their i-th element.

All vectors must be the same length, unless a vector has length 1, in which case it is recycled across all iterations. The full list remains accessible as `.input` inside each branch.
