...
For use with `each()` in `@maestroInputs`. When the upstream pipeline returns a named list, specifies which field of the list to scatter over. The full list remains accessible as `.input` inside each branch, so other fields can still be used.

The value must follow the pattern `.input$field`.
