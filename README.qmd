...
asciicast::init_knitr_engine(
  ...
  startup = quote({
    library(maestro)
    options(maestro.check_datetime_override = as.POSIXct("2026-04-21 00:00:00", tz = "UTC"))
    set.seed(1) 
  })
)
...
A pipeline is a task we want to run. This task may involve retrieving data from a source, performing cleaning and computation on the data, then sending it to a destination. `maestro` is not concerned with what your pipeline does, but rather *when* you want to run it. Here's a simple pipeline in `maestro`:
